...
def encode_position(fen):
  ...
  res[:, :, 12] = board.turn
  res[:, :, 13] = getCastlingRights(board)
  res[:, :, 14] = -1 if board.ep_square is None else board.ep_square
  return res



def store_many_hdf5(images, labels, tag=""):
  num_images = len(images)

  # Create a new HDF5 file
  file = h5py.File(f"{num_images}_position{tag}.h5", "w")
  ...
  meta_set = file.create_dataset(
      "label", np.shape(labels), h5py.h5t.STD_U8BE, data=labels
  )
  file.close()

def read_many_hdf5(num_images, tag=""):
  file = h5py.File(f"{num_images}_position{tag}.h5", "r+")
