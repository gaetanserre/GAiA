...
inline tensor scale_tensors(const tensors& ts_all) {
  ...
  float_vec result_values;
  result_values.reserve(ts_values.front()->size());
  int depth = get_tensor_shape_dimension_by_index(ts.front().shape(), 4);
  for (std::size_t i = 0; i < ts_values.front()->size(); i++) {
    result_values.push_back((*ts_values[0])[i] * (*ts_values[1])[i % depth]);
  }
  return tensor(ts.front().shape(), std::move(result_values));
}
